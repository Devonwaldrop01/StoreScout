...
(P/'supplemental-observations').mkdir(exist_ok=True)
...
def observe(domain,attempt=1):
 path=P/'supplemental-observations'/(hashlib.sha256(domain.encode()).hexdigest()+'.json')
 if path.exists() and attempt==1:return json.loads(path.read_text())
 original=P/'public-observations'/path.name
 if original.exists() and attempt==1:return json.loads(original.read_text())
 ...
 if attempt>1 and path.exists():o['previous_attempt']=json.loads(path.read_text())
...
with concurrent.futures.ThreadPoolExecutor(max_workers=12) as pool:list(pool.map(lambda d:observe(d,2),retry))
