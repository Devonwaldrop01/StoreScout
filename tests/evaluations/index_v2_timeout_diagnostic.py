...
def log(event, **values):
    print(json.dumps(dict(event=event,
                         utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),
                         **dict({'elapsed':round(time.monotonic()-START,6)}, **values))),
...
def child():
    ...
    for name in ('_TITLE_RE', '_META_DESC_RE', '_META_DESC_RE2', '_OG_SITE_RE',
        ...
        if hasattr(probe, name): setattr(probe, name, TimedPattern(name, getattr(probe, name)))
